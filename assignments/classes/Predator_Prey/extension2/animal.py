...
class Animal():
    ...
    def _create_offspring(self):
        '''Tries to create and offspring: returns True if an offspring could be created, else False.'''
        success = False
    ...
    def breed(self, num_offspring=1):
        '''Breed num_offspring new Animals'''
        if self._breed_clock <= 0:
            for _ in range(num_offspring):
                success = self._create_offspring()
                if not success:
                    break
